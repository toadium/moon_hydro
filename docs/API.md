...
| `XinanjiangModel::step` | `(Self, ForcingData, XinanjiangState) -> XinanjiangState` | 单步仿真（V0.9.3修复：保持汇流状态qs/qi/qg） |
...
| e | Double | 时段蒸散发量 |
| q | Double | 出口断面流量 |
| **qs/qi/qg** | **Double** | **V0.9.3新增：地表水/壤中流/地下水汇流分量，修复逐步仿真路由状态丢失** |

### HydroModel Trait（V0.9.3更新）
...
> V0.9.3说明：当前trait签名绑定XinanjiangState，未来泛化方案：引入ModelState trait或关联类型。
...
| `SWEGrid1D::step_lf` | `(Self, Double, SWEBoundary) -> SWEGrid1D` | 单步Lax-Friedrichs推进（V0.9.3：不可变边界，返回新网格） |
| `SWEGrid1D::run` | `(Self, SWESolverConfig) -> SWESolverResult` | 执行 1D 求解（CFL自适应步长） |

> V0.9.3变更：`apply_boundary`返回网格副本不修改输入；Manning阈值从0.000001提高到0.001避免极浅水不稳定。
...
> V0.9.3变更：`couple_run`状态序列包含qs/qi/qg字段；`run_couple` JSON端点正确返回runoff_series。
...
> V0.9.3：SCE-UA支持target_metric配置("nse"或"kge")，不再硬编码NSE。
...
| r | Double | 扰动率(默认0.2) |
| seed | UInt64 | 随机种子 |
| target_metric | String | 目标指标("nse"或"kge")，V0.9.3修复：DDS正确传递此参数 |
| params | Array[CalibParam] | 待率定参数 |
| **max_no_improve** | **Int** | **V0.9.3新增：最大无改进迭代次数(默认100)，替代硬编码收敛阈值** |
...
| **has_obs** | **Bool** | **V0.9.3新增：标记是否有观测数据，替代nse!=0过滤** |
...
| `delete_file` | `(String) -> Unit raise` | 删除文件（V0.9.3新增C FFI） |
| `hydro_file_exists` | `(String) -> Bool` | C FFI文件存在检查（V0.9.3新增） |
| `hydro_delete_file` | `(String) -> Int` | C FFI文件删除（V0.9.3新增） |
...
> 密码哈希使用 FNV-1a + splitmix 多轮哈希（V0.9.3增强），Token 使用 splitmix 伪随机生成。
...
*生成时间：2026-09-03｜MoonBit 0.1.20260827｜230+ 个公开函数｜415+ 测试通过｜V0.9.3*